...
def chat_demo() -> None:
    ...
    with st.expander("Scenario Settings"):
        ...
        if (agent_choice_1 or agent_choice_2 or user_position or scenario_choice) and not st.session_state.active:
    ...
    with start_col:
        ...
        if start_button:
            ...
            st.session_state.state = ActionState.HUMAN_WAITING if user_position == "First Agent" else ActionState.MODEL_WAITING
    ...
    with st.form("user_input", clear_on_submit=True):
        ...
        if st.form_submit_button("Submit", use_container_width=True, disabled=st.session_state.state != ActionState.HUMAN_WAITING):
    ...
    with st.expander("Background"):
        ...
        for agent_idx, agent_info in enumerate(agent_infos):
            ...
            f"""
                <details>
                    <summary>Agent {agent_idx} Background</summary>
                    {agent_info}
                </details>
            """,
    ...
    with st.expander("Chat History"):
        pretty_print_messages(compose_messages())
...
def compose_agents() -> list[str]: #type: ignore
    ...
    agent_to_render = [_agent_profile_to_friendabove_self(agent.profile, agent_id) for agent_id, agent in enumerate(agents.values())]
...
def compose_messages() -> list[str]: # type: ignore
    ...
    ep_to_render = epilog.render_for_humans()[1][:-2]
    ep_to_render[0] = ep_to_render[0].split("Conversation Starts:")[-1].strip()
    
    return ep_to_render
    
            
def set_settings(agent_choice_1, agent_choice_2, scenario_choice, user_position) -> None: # type: ignore
    ...
    ]] if st.session_state.messages == [] else st.session_state.messages